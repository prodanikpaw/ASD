...
TEST(Iterator_test, empty_list_iteration) {
    List<int> list;  // Пустой список

    // Проверяем, что begin() == end() для пустого списка
    EXPECT_TRUE(list.begin() == list.end());

    // Цикл по пустому списку не должен выполниться ни разу
    ...
    // Проверяем итераторы после добавления элементов
    list.push_back(42);
    EXPECT_FALSE(list.begin() == list.end());

    // Проверяем доступ к единственному элементу
...
TEST(Iterator_test, can_write_through_iterator) {
    List<int> list;
    ...
    // Модифицируем значения через итератор
    int multiplier = 10;
    for (List<int>::Iterator it = list.begin(); it != list.end(); ++it) {
        *it = *it * multiplier;
    }
    ...
    List<int>::Iterator it = list.begin();
    EXPECT_EQ(*it, 0);   // 0 * 10 = 0
    ++it;
    EXPECT_EQ(*it, 10);  // 1 * 10 = 10
    ++it;
    EXPECT_EQ(*it, 20);  // 2 * 10 = 20
    ++it;
    EXPECT_EQ(*it, 30);  // 3 * 10 = 30
    ++it;
    EXPECT_EQ(*it, 40);  // 4 * 10 = 40
